...
export function SearchResults() {
  ...
  const headerHeight = isMobile ? 150 : 175;
    ...
    <div style={{ minHeight: '100vh', background: '#F7F8FA', paddingBottom: isMobile ? 80 : 40 }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          {/* Input localisation — flex-1, texte tronqué */}
          <div style={{ position: 'relative', flex: 1, minWidth: 0 }}>
            <MapPin style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', width: 14, height: 14, color: '#9CA3AF', flexShrink: 0 }} />
            <input
          ...
          {/* Filtres rapides — desktop uniquement */}
          {!isMobile && (
            <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
              ...
              <button style={{ padding: '7px 14px', border: '1.5px solid #E5E7EB', borderRadius: 20, fontSize: '1.2rem', fontWeight: 500, color: '#1B4FD8', background: '#EEF2FF', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
                <SlidersHorizontal style={{ width: 14, height: 14 }} /> Filtres
              </button>
            </div>
          )}

          {/* Filtres icône — mobile uniquement */}
          {isMobile && (
            <button style={{ padding: '8px', border: '1.5px solid #E5E7EB', borderRadius: 8, background: '#EEF2FF', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <SlidersHorizontal style={{ width: 18, height: 18, color: '#1B4FD8' }} />
            </button>
          )}
        </div>
        ...
        <div style={{ padding: isMobile ? '6px 12px' : '8px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ position: 'relative' }}>
            <button onClick={() => setShowSortMenu(!showSortMenu)}
              ...
              {isMobile ? '' : (sortBy === 'recent' ? 'Récent' : sortBy === 'prix-asc' ? 'Prix ↑' : sortBy === 'prix-desc' ? 'Prix ↓' : 'Note')}
              {isMobile && <span>{sortBy === 'recent' ? 'Récent' : sortBy === 'prix-asc' ? 'Prix ↑' : sortBy === 'prix-desc' ? 'Prix ↓' : 'Note'}</span>}
        ...
        {/* Ligne 3 : filtres pills — scroll horizontal */}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}>
            <div style={{
              ...
              <button style={{ background: 'none', border: 'none', fontSize: '1.3rem', fontWeight: 600, color: '#1B4FD8', cursor: 'pointer', marginLeft: 'auto', whiteSpace: 'nowrap', flexShrink: 0 }}>
                Voir tout →
              </button>
            </div>
          </div>
            ...
            sorted.map(listing => (
              <div key={listing.id}
                ...
                <div style={{
                  ...
                  <img src={listing.image} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', minHeight: isMobile ? 0 : 180, display: 'block' }} />
                  ...
                  <div style={{ position: 'absolute', top: 10, left: 10, display: 'flex', flexDirection: 'column', gap: 5 }}>
                    ...
                    <span style={{ background: BADGE_COLORS[listing.type], color: '#fff', fontSize: '1.1rem', fontWeight: 700, padding: '3px 10px', borderRadius: 20, alignSelf: 'flex-start' }}>{listing.type}</span>
                  </div>
                ...
                <div style={{ flex: 1, padding: isMobile ? '14px 16px' : '16px 18px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                  <div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4, color: '#9CA3AF' }}>
                      ...
                      <span style={{ fontSize: '1.2rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{listing.commune}, {listing.wilaya}</span>
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: 8, marginTop: 14, alignItems: 'center' }}>
                    <button onClick={e => e.stopPropagation()}
                      style={{ flex: 1, background: '#1B4FD8', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 0', fontSize: '1.3rem', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                      <Phone style={{ width: 14, height: 14 }} /> Appeler
                    </button>
                    <button onClick={e => e.stopPropagation()}
                      style={{ flex: 1, background: '#F0FDF4', color: '#16A34A', border: '1.5px solid #86EFAC', borderRadius: 8, padding: '10px 0', fontSize: '1.3rem', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
          ...
          {/* CTA compact — mobile uniquement (à la place de la sidebar) */}
          {isMobile && (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16, position: 'sticky', top: 185 }}>
            ...
            <div style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 14, padding: '16px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {RECOMMENDED.map(r => (
                  <button key={r} style={{ textAlign: 'left', background: 'none', border: 'none', color: '#1B4FD8', fontSize: '1.2rem', cursor: 'pointer', padding: '4px 0' }}>
                    {r}
                  </button>
                ))}
              </div>
